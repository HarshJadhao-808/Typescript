...
const Input = () => {
  ...
  return (
		<div className="h-100 w-[50%]  border-  grid items-center  ">
			...
			<form onSubmit={sendIt} className="flex justify-center flex-col border-4 w-80 sm:w-120  h-full gap-3 sm:gap-5 ">
				<span className="grid ">
					<label className="text-center mb-3 font-medium text-2xl">Task</label>
					<input onChange={Changes} name='task' className="border-2 h-8 w-60 m-auto  text-center " type="text" />
				</span>
				<span className="grid ">
					<label className="text-center mb-3 font-medium text-2xl">Priority</label>
					<select onChange={Changes} name='priority' className="border-2 h-8 w-60 m-auto ">
